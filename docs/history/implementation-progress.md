...
- 分支 `feat/quiet-diagnostics-mcp-files`，分階段提交診斷呈現、MCP 文件管理、移除 demo／封裝，以及回歸文件。
- 共用診斷呈現控制器保留即時解析；文字變更隱藏編輯範圍的波浪、錯誤 hover 與自動燈泡，800ms 暫停或離開該行後發布。組字不強制發布；問題列表、數量及圖表標記一起使用已發布结果。Monaco、閱讀編輯及節點內編輯均已接入。
- 新增 7 個 MCP 文件工具，合計 20 個；目錄、空資料夾及垃圾桶快照保護、預覽、操作去重、同名與邊界檢查、完整資料夾／非 Yarn 檔案及文件 ID 保留。新增與復原不導航；新增／改名的操作日誌可在磁碟已改動、metadata 失敗後辨識已套用並恢復。
...
- 分支 `feat/mcp-integration`，按工作階段／情境 → 語義交易 → 本機 server／設定 → 回歸文件分階段提交。
- 13 個 MCP Tools：多專案／同專案多視窗明確目標、已知專案開啟、即時來源／閱讀／圖表情境、查詢、診斷、統計、指令管理、文字交易及指定快速修正。Activate／新視窗預設不搶焦點，失效 ID 不回退其他目標。
- 官方 SDK Streamable HTTP 僅監聽 loopback，預設停用、唯讀／修改模式、profile 獨立憑證及固定連接埠；設定頁支援複製連線資料與撤銷。SDK 與實際打包依賴的授權檔隨 desktop staging 提供。
...
- 設定與指令 tab 切換僅隱藏，分類、捲動、查詢及表單留在記憶體；關閉指令頁直接捨棄未套用內容。
- 指令草稿移出工作區 action、profile 及備份，舊資料殘留欄位讀取時移除；已套用定義與文件草稿保存照舊。
- 161 項單元／服務測試、TypeScript、lint、桌面 build 通過；隔離 profile 實測切換、設定捲動、關閉再開及保留已套用定義，頁面錯誤為零。現已納入 0.9.1 Portable。

## 已納入 0.9.1：指令快速修正與編輯器浮層（2026-09-21）

- 專案資料目錄改為 `.spindle`，依需求不相容舊目錄。
- 三種編輯模式支援未註冊命令 hover「新增指令」及 Alt+Enter，去重保存並保留原文。
- 診斷外層、快速修正、Find／Replace 與相關編輯器浮層採共用圓角、邊框、控制項及配色；閱讀搜尋中文化。
- 160 項單元／服務測試、型別檢查、lint、Web／桌面 build 通過；隔離 profile 桌面互動驗證三模式註冊、Find 幾何及原文不變。現已納入 0.9.1 Portable，Setup 不更新。

## 0.9.0 初始畫面、專案管理與復原

- 獨立初始畫面、可搜尋完整專案列表、最近 5 筆、行內專案改名、建立同名子資料夾，以及預設關閉的上次專案重開設定已實作。初始畫面亦提供全 App 共用的編輯器風格設定。
...
- 桌面 build 與隔離 profile 實測通過：按鈕中心差不超過 1px、持續 hover 無浮窗、直接點擊可關閉，頁面錯誤為零。現已納入 0.9.1 Portable。


### 已納入 0.9.1：編輯器風格與全局繼承（2026-09-21）

- 新增全局與四模式逐欄覆寫、系統字型搜尋／手動輸入、字級／倍數行高、內容配色與純文字語法及編輯輔助設定；設定分類、模式與 tab 保留輸入和捲動。
- App profile 共用設定、逐欄序列化更新及舊閱讀偏好單次遷移；Web 獨立保存並同步偏好。文字與節點編輯使用動態配置，不重建編輯器；圖表量測更新保持手動布局。
- 驗證入口：`test:unit`、`test:appearance`、`test:tab-lifecycle`、`test:find-controls`。實測與 build 結果見本輪 `outputs/appearance-*.log`；現已納入 0.9.1 Portable。未實測：非 Windows 系統字型枚舉、多螢幕／不同 DPI 切換、真實 IME 跨窗組字。

- 本輪追加：分離游標符號關聯與選取文字匹配，Find 查詢優先；不透明度改獨立棋盤格軌道及百分比。單元測試 170 項通過；UI 回歸另涵蓋高亮互斥、透明度端點、百分比與 RGB 分離。未重新打包安裝版。

- 游標符號關聯新增底線／背景高亮樣式；多行顏色設定改為帶標頭的分組，來源選單與子欄位分層。171 項單元測試與桌面樣式切換回歸通過。

- 未宣告 `set` 改為專案範圍錯誤檢查，新增宣告 hover／Alt+Enter 支援三種編輯器；變數關聯高亮支援跨檔案宣告，診斷 hover 優先於指令說明。回歸入口 `test:variable-quick-fix`，現已納入 0.9.1 Portable。


### 變數語義補強（已納入 0.9.1）
- 依專案宣告檢查 set 型別；變數提示保留初始值與來源，三種編輯器共用跨檔案 Ctrl 宣告跳轉。
- 診斷更新涵蓋外部宣告修改；新增 test:variable-semantics 桌面回歸。
- 驗證：176 項單元測試、型別檢查、lint、Web／桌面 build；隔離 profile 的變數 Hover／跨檔案 Ctrl 跳轉、宣告 quick fix 與 Undo 回歸通過。現已納入 0.9.1 Portable；未涵蓋未知函式回傳型別推導。


### 指派與編輯互動修正（已納入 0.9.1）
- 裸文字指派診斷及「補上雙引號」Hover／Alt+Enter 快速修正，適用純文字、閱讀編輯與圖表內編輯。
- 純文字游標高度與行距分離；全局搜尋帶入編輯器當前選取文字。
- 驗證：178 項單元測試、型別檢查、lint、Web／桌面建置通過；隔離 profile 桌面回歸涵蓋搜尋詞、補引號、Undo、游標高度與既有跨檔變數導航。

- 指派診斷優先順序補正：裸文字指派 number／boolean 優先型別錯誤；只有 string 指派才提供補引號提示與修正。


### 補全與快速修正擴充（已納入 0.9.1）
- declare 補全帶入空格及 $；變數前綴、型別錯誤與缺值共用快速修正，支援指令參數與跨檔已宣告變數。
- 場景目標 Hover 顯示檔案／行號；純文字及 CodeMirror 反白縮短繪製高度，保留多行選取範圍。
- 180 項單元測試、型別、lint 及 Web／桌面建置；隔離桌面測試驗證三種編輯器修正、補全、場景提示與選取樣式。
