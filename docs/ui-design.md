...
狀態：**0.6.4 靜默操作與語言輔助修訂。** 更新日期：2026-09-20。適用於深色 Windows 桌面與網頁工作區。
...
- 劇本「移到垃圾桶」直接執行，先留復原副本；磁碟文件送至系統垃圾桶，失敗保留文件。與保留原檔的「從專案移除」分開。
...
- `set` 的第一個位置提供專案變數候選；條件與運算式輸入 `$` 查找。候選顯示型別與來源文件；有 /// 說明時顯示旁側描述。補全選項不得被參數提示遮住，選取後恢復當前參數提示。
