...
狀態：**0.8.0 持久圖表布局與手動佈線。** 更新日期：2026-09-20。適用於深色 Windows 桌面與網頁工作區。
...
- 劇本「移到垃圾桶」直接執行，先留復原副本；磁碟文件送至系統垃圾桶，失敗保留文件。0.7.0 起移除「從專案移除」選單入口。
...
### 0.8.1 持久布局與手動佈線（使用者核准，2026-09-20）

本節取代 0.7.x 的即時計算路由與分支埠規則；0.8.1 依新回饋取代卡片沿舊路徑滑動的限制。
...
- 單擊線高亮並顯示控制點，共用幹線選整組。雙擊獨立線新增 pin；水平線段上下移、垂直線段左右移。卡片、幹線、pin 具有明確操作層，透明線段命中區不能攔截上層可見控制點。
- 卡片可自由移動、框選與混合多選；卡片和 pin 的位置決定線路，先後順序按連線保留。拖動即時顯示路徑，放開修整避障，只重算受影響線。pin 右鍵提供「刪除 pin」，Delete 只刪選取 pin；「簡化線路」保留 pin、「恢復自動線路」清除該路線手動限制，不刪劇情連線。
- 拖曳場景、卡片、pin、線段、幹線或右鍵平移時，游標顯示 grabbing；放開／Esc／取消恢復。單擊不新增布局歷史。一次拖曳或整理是一筆完整布局歷史，Esc 還原開始狀態；整組內部線、pin、卡片隨組搬移，跨組保留外部幾何。
- 保存安靜；只有引擎錯誤／逾時或固定路線衝突顯示重試。歷史、幾何與視野按 tab／DocumentId 保存。
