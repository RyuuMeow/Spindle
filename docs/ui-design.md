...
狀態：**0.4.0 桌面互動修訂已交付，portable 回歸通過；原生操作驗證限制另列。** 更新日期：2026-09-19。適用於深色 Windows 桌面與網頁工作區。

本文件是最新設計規則；實際發行與最終測試結果以[實作追蹤](implementation-progress.md)為準。本輪依據使用者對 0.3.0 的實際回饋、R01–R07 修訂及「正文以外物件簡單低調」要求，取代第一輪的搜尋側欄、指令 tab、可釘選大綱與緊湊命令行框。原始根因與汰換關係保留於[設計複審](desktop-design-review-2026-09-18.md)。

## 實作與驗證狀態

| 範圍 | 已驗證 | 保留邊界 |
|---|---|---|
| 閱讀節奏 | 18項單元、CodeMirror幾何、跨區塊編輯及桌面整合 | 原生中文 IME 未實測 |
| 文件導航與建立 | 目前TabId、獨立歷史、行內新增、排序、磁碟失敗回滾 | 原生跨窗拖曳另驗收 |
| 指令與搜尋 | 浮層入口、焦點、快捷鍵隔離、失焦關閉、結果定位及草稿重開 | 組字防護採合成事件 |
| 節點直接編輯 | 共用來源／Undo、跨檔、失效範圍及未提交草稿恢復 | 不含圖上拉線改寫 |
| 面板與控制項 | 800／1100／1440px、鍵盤尺寸調整、來源位置及溢位 | 混合DPI／讀屏另驗收 |
| 發行 | 以[實作追蹤](implementation-progress.md)的新包證據為準 | 不以0.3.0結果替代 |
...
| 自訂指令 | 獨立大型 modal，背景壓暗並停止互動；不建立文件 tab，關閉返回原稿 |
...
- 指令面板保留周圍背景，約 1120px 寬上限、最大尺寸受視窗限制；標題／專案身份固定，內部清單與定義獨立捲動。背景使用暗遮罩，表面用低對比描邊和柔影。
- 指令參數採緊湊列，「套用定義」位於表單頂部；未更動時停用。副本／Undo／Redo 用帶提示 SVG，刪除置於更多選單。未完成／非法定義保留草稿，關閉不自動套用。
- 指令 modal 限定焦點範圍；關閉恢復來源焦點。組字中的 Escape 留給輸入法；Ctrl+S 處理當前指令，關閉／切 tab／導航快捷鍵不得穿透背景。
...
最終整合至少覆蓋：同稿多 tab 不跳轉、各 tab 歷史獨立；新增 Escape 不留下物件；排序方向及重開狀態一致；搜尋完整鍵盤流程與失焦；指令浮層焦點／快捷鍵隔離；大綱顯示偏好與窄窗避讓；節點直編／跨檔保存／Undo；長名稱與 800／1100／1440px。通過狀態只在實際執行後寫入實作追蹤。
