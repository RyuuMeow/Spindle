...
### 0.8.0 持久布局與手動佈線（使用者核准，2026-09-20）

本節取代 0.7.x 的即時計算路由與分支埠規則。

- 自動整理為一次性動作；首次缺布局才自動執行。無節點選取整理全部，有多選只整理所選；tooltip 顯示範圍。線／卡片選取不算節點選取，局部整理保留未選節點及視野。
...
- 卡片沿所屬水平路徑拖動、吸附同組欄位，不觸發重新佈線。Delete 只刪選取 pin；「簡化線路」保留 pin、「恢復自動線路」清除該路線手動限制，不刪劇情連線。
- 一次拖曳或整理是一筆完整布局歷史，Esc 還原開始狀態；整組內部線、pin、卡片隨組搬移，跨組保留外部幾何。
