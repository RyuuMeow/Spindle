...
### 0.8.3 持久布局與手動佈線（使用者核准，2026-09-20）

本節取代 0.7.x 的即時計算路由與分支埠規則；0.8.1 取代卡片沿舊路徑滑動的限制；0.8.2 依使用者回饋取消線段／幹線拖曳與路線衝突提示；0.8.3 加入小幅吸附與依目的地分線。
...
- 不同目的地的水平／垂直共線段以 16 單位平行 offset 分開；只有同方向、同目的地且通過最後卡片／pin 後的末段允許匯流。來源短幹線保留，分岔後不以不同目的地共線製造假匯流。分線尊重 pin、卡片、中心埠及障礙，不為分線搬動節點；重疊的硬性控制點不強制移開。拖動預覽與放開後共用規則，未受影響線路逐點保留。
