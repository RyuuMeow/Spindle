...
狀態：**0.6.1 指令輸入情境與摺疊對齊修訂。** 更新日期：2026-09-19。適用於深色 Windows 桌面與網頁工作區。
...
- 指令提示：標題顯示名稱、次要原始識別字；描述另起一段；參數用位置序號、名稱、型別、選填與縮排說明對齊。範例置於底部。閱讀／節點使用共用 DOM 浮層，純文字與虛擬參數使用安全 Markdown 表格。長描述允許換行，浮層限制寬高並可捲動，不把專案文字當 HTML。
