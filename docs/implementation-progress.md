...
- 原生中文 IME、混合 DPI／多螢幕、所有第三方 client 的設定 UI、安裝包尚未實測；本輪不產出新 Setup／Portable。完整工具與邊界見 [MCP 接入](mcp.md)。

## 待打包：分頁狀態與指令草稿生命週期（2026-09-21）

- 設定與指令 tab 切換僅隱藏，分類、捲動、查詢及表單留在記憶體；關閉指令頁直接捨棄未套用內容。
- 指令草稿移出工作區 action、profile 及備份，舊資料殘留欄位讀取時移除；已套用定義與文件草稿保存照舊。
- 161 項單元／服務測試、TypeScript、lint、桌面 build 通過；隔離 profile 實測切換、設定捲動、關閉再開及保留已套用定義，頁面錯誤為零。尚未重新打包。

## 待打包：指令快速修正與編輯器浮層（2026-09-21）
...
- 160 項單元／服務測試、型別檢查、lint、Web／桌面 build 通過；隔離 profile 桌面互動驗證三模式註冊、Find 幾何及原文不變。既有 Portable／Setup 尚未包含本輪修改。
...
- 驗證入口：`test:unit`、`test:appearance`、`test:tab-lifecycle`、`test:find-controls`。實測與 build 結果見本輪 `outputs/appearance-*.log`；尚未重新打包。未實測：非 Windows 系統字型枚舉、多螢幕／不同 DPI 切換、真實 IME 跨窗組字。
...
- 未宣告 `set` 改為專案範圍錯誤檢查，新增宣告 hover／Alt+Enter 支援三種編輯器；變數關聯高亮支援跨檔案宣告，診斷 hover 優先於指令說明。回歸入口 `test:variable-quick-fix`，未重新打包。


### 變數語義補強（待打包）
- 依專案宣告檢查 set 型別；變數提示保留初始值與來源，三種編輯器共用跨檔案 Ctrl 宣告跳轉。
- 診斷更新涵蓋外部宣告修改；新增 test:variable-semantics 桌面回歸。
- 驗證：176 項單元測試、型別檢查、lint、Web／桌面 build；隔離 profile 的變數 Hover／跨檔案 Ctrl 跳轉、宣告 quick fix 與 Undo 回歸通過。尚未打包；未涵蓋未知函式回傳型別推導。
