...
fs.writeFileSync(path.join(root, ".spindle/project.json"), JSON.stringify({ id: "installation-test", name: "Installation test", commands: [], files: [{ id: "test-document", name: "sample.yarn" }], excluded: [] }));
fs.writeFileSync(path.join(profile, "project-catalog-v1.json"), JSON.stringify({ version: 1, entries: [{ id: "installation-test", name: "Installation test", root, lastOpenedAt: 1, recent: true }], preferences: { reopenLastProject: false } }));
const env = {
  ...process.env,
  USERPROFILE: home,
  HOME: home,
  ...
  try {
    ...
    for (const client of ["codex", "claude"]) {
      ...
      const entry = client === "codex"
        ? require("@iarna/toml").parse(fs.readFileSync(info.configPath, "utf8")).mcp_servers[info.serverName]
        : JSON.parse(fs.readFileSync(info.configPath, "utf8")).mcpServers[info.serverName];
      const { Client } = await import("@modelcontextprotocol/sdk/client/index.js");
      const { StreamableHTTPClientTransport } = await import("@modelcontextprotocol/sdk/client/streamableHttp.js");
      const sdk = new Client({ name: "installed-config-regression", version: "0.9.2" });
      try {
        await sdk.connect(new StreamableHTTPClientTransport(new URL(entry.url), { requestInit: { headers: entry.http_headers || entry.headers } }));
        ...
        const session = (await call("open_project", { projectId: "installation-test" })).sessions[0];
        const target = { editorSessionId: session.editorSessionId };
        await call("reveal_location", { ...target, documentId: "test-document", from: source.indexOf("Hello"), to: source.indexOf("Hello") + 5 });
        const context = await call("get_editor_context", target);
        assert.equal(context.selections[0].text, "Hello");
        const change = { ...target, snapshotId: context.snapshotId, operationId: client + "-readonly", change: { kind: "edits", documents: [{ id: "test-document", version: context.document.version, edits: [{ from: context.selections[0].from, to: context.selections[0].to, insert: "Welcome" }] }] } };
        assert.equal((await sdk.callTool({ name: "apply_changes", arguments: change })).isError, true);
        await page.evaluate(() => window.yarnDesktop.agent.configure({ mode: "write" }));
        assert.equal((await call("apply_changes", { ...change, operationId: client + "-write" })).applied, true);
        let editor;
        for (const candidate of app.windows()) if (await candidate.locator(".monaco-editor").count()) editor = candidate;
        assert.ok(editor);
        await editor.evaluate(async () => {
          const m = await new Promise(resolve => window.require(["vs/editor/editor.main"], resolve));
          const e = m.editor.getEditors().find(e => e.getDomNode()?.offsetParent);
          e.trigger("keyboard", "undo", null);
        });
        await until(async () => (await call("read_document", { ...target, documentId: "test-document" })).text === source);
        await page.evaluate(() => window.yarnDesktop.agent.configure({ mode: "read" }));
      ...
      if (executable) {
        ...
        if (client === "codex") {
          results.codexSkillDiscovered = await require("./codex-skill-check.cjs")(executable, env, home, path.join(info.skillPath, "SKILL.md"));
