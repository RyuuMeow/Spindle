...
module.exports = async function check(executable, env, cwd, expectedPath) {
  const child = spawn(executable, ["app-server"], { env, cwd, windowsHide: true, stdio: ["pipe", "pipe", "ignore"] });
  ...
  lines.on("line", line => {
    ...
    if (result.error) request.reject(new Error("Codex rejected skill discovery")); else request.resolve(result.result);
  });
  const request = (method, params) => new Promise((resolve, reject) => {
    const seq = ++id;
    const timer = setTimeout(() => { pending.delete(seq); reject(new Error("Codex skill discovery timed out")); }, 15000);
  ...
  try {
    await request("initialize", { clientInfo: { name: "spindle-install-test", version: "0.9.2" }, capabilities: { experimentalApi: true } });
    ...
    return skills.some(skill => skill.name === "spindle" && normalize(skill.path) === normalize(expectedPath));
  } finally {
