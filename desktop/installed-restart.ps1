...
try {
 ...
 while ([DateTime]::UtcNow -lt $deadline) {
  if (Test-Path -LiteralPath $job.target) {
   ...
   if ($version -eq $job.version -and (!$job.installer -or !(Get-CimInstance Win32_Process | Where-Object {$_.ExecutablePath -eq $job.installer}))) {
 ...
 $launchArguments = @('--user-data-dir="'+$job.profile+'"') + @($job.arguments | Where-Object { $_ })
 Start-Process -FilePath $job.target -ArgumentList $launchArguments -WindowStyle Hidden
