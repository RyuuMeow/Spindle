...
type AvoidModule = {
  ...
  ConnRef: new (
    ...
    a: Owned,
    b: Owned,
  ) => Owned & { setRoutingType(t: unknown): void; displayRoute(): NativeLine };
...
export async function loadAvoid(wasmUrl?: string): Promise<AvoidModule> {
  if (!loading)
    loading = AvoidLib.load(wasmUrl)
      .then(() => AvoidLib.getInstance() as AvoidModule)
      .catch((error: unknown) => {
        loading = undefined;
        throw error;
      });
...
export async function avoidRoutes(
  obstacles: GraphRect[],
  legs: RouteLeg[],
  wasmUrl?: string,
): Promise<Map<string, Point[]>> {
  const a = await loadAvoid(wasmUrl),
    router = new a.Router(2);
  const temporary: Owned[] = [];
  const point = (p: Point) => {
    const o = new a.Point(p.x, p.y);
    temporary.push(o);
    return o;
  };
  try {
    ...
    router.setRoutingOption(
      a.RoutingOption.nudgeSharedPathsWithCommonEndPoint,
      false,
    );
    router.setRoutingOption(
      a.RoutingOption.nudgeOrthogonalSegmentsConnectedToShapes,
      false,
    );
    for (const box of obstacles) {
      const rectangle = new a.Rectangle(
        point(box),
    ...
    const connectors = legs.map((leg) => {
      const start = new a.ConnEnd(point(leg.from)),
        end = new a.ConnEnd(point(leg.to));
