...
Windows x64 0.4.0 產物在 `release/`：`Yarn-Workbench-0.4.0-Setup-x64.exe` 與 `Yarn-Workbench-0.4.0-Portable-x64.exe`。打包包含離線 Monaco、CodeMirror 與主程序文件服務，不依賴編輯器 CDN。程式未簽章。
...
0.4.0 的全局搜尋是中央偏上的輕量浮層，結果出現在輸入框下方，選取、點外部或失焦即關閉。自訂指令使用壓暗背景的大型面板；設定維持獨立工作區。右側大綱只由工具列切換，左右面板及診斷區可調整尺寸。文件時鐘開啟版本時間線、唯讀預覽與差異比較。
...
一般切劇本、搜尋及大綱定位沿用目前分頁；中鍵、＋或明確新分頁操作才新增視圖。劇本與場景新增／更名在側欄直接輸入，預設名稱反白，Esc 可取消。每個分頁保有独立導航歷史與各稿件閱讀位置。
