...
Windows x64 0.6.3 產物在 `release/`：`Spindle-0.6.3-Setup-x64.exe` 與 `Spindle-0.6.3-Portable-x64.exe`。打包包含離線 Monaco、CodeMirror 與主程序文件服務，不依賴編輯器 CDN。程式未簽章。
...
- 專案名稱、文件識別與有效指令定義寫入 `.yarn-workbench/project.json`；視窗、復原草稿與快照在 App profile。Windows 安裝版及 portable 預設皆使用 `%APPDATA%/Spindle`，portable 不把資料放在 exe 旁。
- v1 專案備份仍可匯入。v2 備份包含相對路徑、指令定義與指令草稿；匯入會新增專案，保留原專案。
- 網頁資料存於該瀏覽器的 localStorage，與桌面 profile 分開；請用專案備份轉移。清除網站資料會移除網頁工作區。

## 編輯與操作

按住 Ctrl 懸浮在可解析的 jump／detour 目標，文字會變色、加底線，Ctrl＋左鍵在目前分頁跳轉；純文字、閱讀與節點編輯一致。指令懸浮提示依名稱、說明、參數與範例分區，虛擬參數標籤也提供對應說明。


0.6.3 的全局搜尋是中央偏上的輕量浮層，結果出現在輸入框下方，選取、點外部或失焦即關閉。自訂指令與設定皆使用獨立工具 tab；指令與參數可設定顯示名稱及說明，閱讀顯示別名，純文字保留原始語法，兩者提供虛擬參數提示與懸浮說明。右側大綱只由工具列切換，左右面板及診斷區可調整尺寸。文件時鐘開啟版本時間線、唯讀預覽與差異比較。
