...
Windows x64 0.7.1 產物在 `release/`：`Spindle-0.7.1-Setup-x64.exe` 與 `Spindle-0.7.1-Portable-x64.exe`。打包包含離線 Monaco、CodeMirror 與主程序文件服務，不依賴編輯器 CDN。程式未簽章。
...
書本按鈕切換純閱讀，僅保留場景、對話與選項；不修改原文。分析按鈕開啟角色／場景統計側欄。資料夾按箭頭展開、單擊選取、雙擊改名，文件與資料夾可跨層拖移；複製後原位改名。流程線採局部更新，拖移時保留有效路徑；既有手排位置保留。檢查無問題時顯示勾號。
