...
Windows x64 0.8.1 產物在 `release/`：`Spindle-0.8.1-Setup-x64.exe` 與 `Spindle-0.8.1-Portable-x64.exe`。打包包含離線 Monaco、CodeMirror、ELK Worker、libavoid WASM、引擎授權檔與主程序文件服務，不依賴編輯器 CDN。程式未簽章。
...
書本按鈕切換純閱讀，僅保留場景、對話與選項；不修改原文。分析按鈕開啟角色／場景統計側欄。資料夾按箭頭展開、單擊選取、雙擊改名，文件與資料夾可跨層拖移；複製後原位改名。圖表首次自動整理，之後只在按下整理時重排全部或所選場景／線上卡片。左鍵框選／Shift 多選，右鍵拖動平移；雙擊線段加 pin，可拖線段、共用幹線與條件卡。卡片可框選、自由移動並與場景一起搬移；路線跟隨卡片與 pin，pin 右鍵或 Delete 可刪除。手動布局、完整 Undo／Redo 隨文件視圖保存；拖動與縮放保留無關路線。檢查無問題時顯示勾號。
